...
\begin{definicia}
    ...
    patriacu $T$ platí $\models A$.  \par Hovoríme, že formula $A$ je
    sémantickým dôsledkom (vetou teórie) množiny formúl $T$,
    označujeme $T \models A$, ak $A$ je splnená v každom modeli teórie
    $T$.
\end{definicia}
...
je taká, že $T \provable A$, potom $T \models A$.
...
pre ľubovoľnú uzavretú formulu $(\exists x)B$ jazyka $L$ platí $$ T \provable
...
(axióma špecifikácie). Potom aj $T \provable B_x[t]$ (lebo teória je Henkinova). $m
\models B[e(x/t)]$.
...
a každá formula $A$ na jazyku $L$, $T \provable A, T' \vdash A$.

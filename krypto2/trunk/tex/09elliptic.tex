...
\todo{ECDSA}
Parametre, ktoré sa zvyknú uvádzať v tabuľkách eliptických kriviek
\begin{itemize}
    \item $A$
    \item $B$
    \item $2P$ -- modulus
    \item $FR$ -- field representation - buď ako polynómy nad $GF(2^m)$
        alebo ako mocniny generátora
    \item $seed$ -- keď niekto neverí vygenerovaným krivkám, môže si
        overiť, že boli vygenerované podľa tohoto seedu.
    \item $G$ -- generátor vhodnej podgrupy, je to bod patriaci krivke
    \item $n$ -- rád generátora \fixme{grupy ci podgrupy?}
    \item $k$ -- kofaktor, $k=|E| / n$
\end{itemize}
\begin{poznamka}
    V štandarde sa používajú len krivky s kofaktorom $k=1$, teda platí
    $G \isomorph Z_p$.
\end{poznamka}

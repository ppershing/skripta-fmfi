...
\paragraph{Definícia:} Nech $L$ je jazyk prvého rádu a $A$ je formula jazyka
$L$. Hovoríme, že formula $A$ je \emph{logicky platná}, označujeme $\models A$,
ak je splnená v ľubovoľnej realizácii jazyka $L$.
$$
\begin{array}{lll}
m & \models & A[e] \\
  & \models & A \\
...
\paragraph{Definícia:} Nech $L$ je jazyk, prvého rádu a $T$ je množina formúl
jazyka $L$. Hovoríme, že $T$ je teória 1. rádu predikátovej logiky s jazykom
$L$ (množina formúl $T$ je množina axióm teórie).

\paragraph{Definícia:} Nech $T$ je teória v jazyku $L$, $m$ je realizácia jazyka
$L$. Hovoríme, že $m$ je modelom teórie $T$, ak pre každú formulu $A$ patriacu
$T$ platí $\models A$.
\par Hovoríme, že formula $A$ je sémantickým dôsledkom (vetou teórie) množiny formúl $T$,
označujeme $T \models A$, ak $A$ je splnená v každom modeli teórie $T$.
...
\paragraph{Príklad:} Elementárna aritmetika.
$$+, \cdot, 0, S(x) = x+1, N^+ $$
\begin{enumerate}
	\item $\neg S(x) = 0$
...
Pätica v zátvorkách je \emph{štandardný model} elementárnej aritmetiky:
$$<N^+, +, \cdot, S, 0>$$
...
\paragraph{Veta (o korektnosti):} Ak $T$ je teória v jazyku $L$ a ak formula $A$
je taká, že $T \vdash A$, potom $T \models A$.
\paragraph{Dôkaz:} Nech $A_1, A_2, \ldots A_n$ je odvodenie (dókaz) formuly $A$
z predpokladov $T$ (v teórii $T$). $A_1, A_2, \ldots A_n(A)$.
\par $m$ nech je ľubovoľný model teórie $T$. Ukážeme (indukciou podľa dĺžky
dôkazu), že platí $m \models A_i$ pri predpoklade, že pre $A_j, j < i$ platí
$m \models A_j$.i

Do dôkazu sa $A_i$ môže dostať niekoľkými spôsobmi:
\begin{enumerate}
	\item $A_i \in T$, m -- model $T$, potom $m \models A_i$
	\item $A_i$ sa dostane do dôkazu ako schéma axiom predikátovej logiky:
	\begin{enumerate}
		\item $A_i$ je axioma výrokovej logiky -- je poskladaná z
		atomických formúl a logických spojok $\neg$ a $\implies$. Potom
		$A_i$ je tautológia výrokovej logiky (ak formula je tautoĺógia,
		...
		\models A_i$.
		\par
		Zaujíma nás prípad, kedy $(\forall x) B$ je pravdivý. To
		znamená, že pre ľubovoľné ohodnotenie platí $B[e(x/m)]$, teda
	...
	\begin{enumerate}
		...
		keď  $e(x_i/m_i)$ a $e(y_i/m_i)$. Získavame potom zobrazenie na
...
\paragraph{Pokračovanie vety o korektnosti:}

\paragraph{Príklad:} Uvažujme elementárnu aritmetiku, ktorá má svoj štandardný
...
Formula ani jej negácia nie sú dokázateľné v elementárnej aritmetike.
...
Ak $L$ je jazyk bez rovnosti a $\vdash^L A$, potom $A^*$ je tautológia. Ak $L$
je jazyk s rovnosťou a $\vdash^L A$, potom $A^*$ je tautologický dôsledok $c=c$.

\par Tvrdíme, že pre žiadnu formulu $A$ nie je $\vdash A$ aj $\vdash \neg A$. Ak
by to platilo, dostali by sme sa do sporu, že vo výrokovej logike je $\vdash
A^*$ aj $\vdash \neg A^*$.


\section{Veta o úplnosti}
\paragraph{Veta (G\"odel, 1):} Nech $T$ je teória v jazyku $L$ a ak $A$ je
ľubovoľná formula jazyka $L$, potom $T \vdash A \iff T \models A$.

\paragraph{Veta (G\"odel, 2)} Teória $T$ je bezosporná práve vtedy, ked $T$ má
model.
...
\begin{enumerate}
	...
	\item Ak jazyk $L$ bude jazyk s rovnosťou, môže sa stať, že v teórii $T$
		bude $T \vdash t=s$, ale $t$ a $s$ sú rôzne termy bez
		premenných.
...
\paragraph{Definícia:} Hovoríme, že teória $T$ s jazykom $L$ je \emph{úplná}, ak
je bezosporná teória a pre ľubovoľnú uzavretú formulu $A$ na jazyku $L$ buď $A$,
...
\paragraph{Definícia:} Hovoríme, že teória $T$ s jazykom $L$ je \emph{Henkinova}, ak
pre ľubovoľnú uzavretú formulu $(\exists x)B$ jazyka $L$ platí $$ T \vdash
(\exists x)B \implies B_x[c]$$ pre nejaké $c$ -- konštantu.

\paragraph{Lema:} Ak $T$ je úplná a Henkinova teória, tak potom $T$ má model.
\paragraph{Dôkaz:} Nech $L$ je jazyk teórie $T$, $\tau$ je množina všetkých
termov jazyka $L$ bez premenných. Na množine $\tau$ definujem reláciu
ekvivalencie nasledovne:
$$
	t_1, t_2 \in \tau
	t_1 \equiv t_2 \iff T \vdash t_1 = t_2
$$
Rovnosť je reflexívna, symetrická, tranzitívna, teda týmto spôsobom definovaná
relácia je relácia ekvivalencie a rozdeľuje nám množinu $\tau$ na triedy
ekvivalencie:
...
\paragraph{Lema:} Ak $T$ je úplná a Henkinova teória, tak potom $T$ má model.
\paragraph{Dôkaz:} $M$ bude mnomžina všetkých termov bez premenných. Na tejto
množine definujeme reláciu $\equiv$ tak, že 
$$ t_1 \equiv t_2,~t_1, t_1 \in \tau,~T \vdash t_1=t_2 $$
$$[t]=\{ s\in \tau, s \equiv t \} $$

Nech $f$ je $n$-árny funkčný symbol. $[t_1], [t_2], \ldots, [t_n] \in M$, pričom
platí $f_m([t_1], \ldots, [t_n]) = [f(t_1, \ldots t_n)]$. $P$ je $n$-árny
predikátový symbol rôzny od $=$. $([t_1], \ldots [t_n]) \in P_m \iff T \vdash
P(t_1, \ldots, t_n)$.
...
$$ [t_1], \ldots, [t_n]) \in P_m \iff T \vdash P(s_1, \dots s_n) $$
...
$A$ -- ľubovoľná uzavretá formula z $L$, potom $m \models A \iff T \vdash A$.
$P(t_1, t_2, \ldots t_n)$ -- atomická formula,  uzavretá, $t_1. \ldots t_n$ sú
termy bez premenných.

$m \models A \iff ([t_1], \ldots, [t_n]) \in P_m \iff T \vdash P(t_1,\ldots
t_n)$ pre ľubovoľné ohodnitenie.

\par $A: t_1 = t_2$. $m \models t_1 = t_2 \iff [t_1] = [t_2] \iff T \vdash t_1 =
t_2$.

\par $A: \neg B$. Indukčný predpoklad: pre $B$ bolo tvrdenie dokázané ($m\models
B \iff T \vdash B)$. Formula $A$ je pravdivá v $m$ práve vtedy, keď formula $B$
nie je pravdivá $B$ nie je pravdivá, $T \nvdash B, T \vdash \neg B$.
...
$$ T \vdash B \implies C $$
$$ \vdash C \implies (B \implies C)$$
Opačná implikácia:
$$T\vdash B\implies C\Rightarrow m \models B \implies C$$

Teória $T$ je úplná teória a $B \implies C$ je uzavretá. Pre každú uzavretú
formulu platí, jedna z možností $T \vdash \neg B$ (potom $m \models \neg B$)
alebo $T \vdash B$, a z toho vyplýva, že $T \vdash C$, a teda $m \models C$.
Tvrdenie je teda dokázané.

$$m \models A \iff T \vdash A$$
\par Predpokladajme, že formula $A$ je v tvare $A: (\forall x) B$. Pre každú
inštanciu formuly $B$ tvrdenie platí, $\vdash (\forall x) B \implies B_x[t]$
(axióma špecifikácie). Potom aj $T \vdash B_x[t]$ (lebo teória je Henkinova). $m
\models B[e(x/t)]$.

\par Ak je pravdivá negácia, potom $T \vdash \neg A$, čo znamená, že $(\exists
x) \neg B$. $T \vdash (\exists x) \neg B \implies B_x[c]$, a to znamená, že $T
\vdash \neg B_x[c]$.

\par Nech $A$ je ľubovoľná formula z teórie $T$. Pre ňu platí, že $T \vdash A$
(predpoklad). Potrebujem dokázať, že $T \vdash A \iff m \models A$. Zoberiem si
$A'$ -- uzáver formuly $A$. Na $A'$ sa vzťahuje tvrdenie, ktoré sme už dokázali,
a teda $m \models A' \iff T \vdash A'$. Podľa vety o uzávere:
$m \models A \iff  m\models A' T \vdash A' \iff T \vdash A$
...
\paragraph{Definícia:} Jazyk $L'$ je \emph{rozšírením jazyka} $L$, ak každý
špeciálny symbol jazyka $L$ (prípadne aj symbol $=$) je v jazyku $L'$ obsiahnutý
s rovnakým významom a s rovnakou árnosťou.

\paragraph{Príklad:} $L'$ s rovnosťou a špeciálnymi symbolmi $<$ a $0$. Jazyk
$L$ má symboly $<$ a $=$. Jazyk $L'$ je rozšírením jazyka $L$

\paragraph{Definícia:} Majme teóriu $T'$ s jazykom $L'$. Hovoríme, že je
rozšírením teórie $T$ s jazykom $L$, ak platí, že $L'$ je rozšírením jazyka $L$
a každá formula $A$ na jazyku $L$, $T \vdash A, T' \vdash A$.
\paragraph{Veta (Henkinova):}
...
\par $T \vdash A$. $A'$ -- uzáver A. $$ m \models A' \iff T \vdash A'$$. Ak v
...
by sa našla taká formula, že $T \vdash A$ aj $T \vdash \neg A$, z čoho vyplýva
$T' \vdash A$ aj $T' \vdash \neg A$.

\paragraph{Definícia:} Majme teóriu $T$ a teóriu $T'$. Hovoríme, že $T'$ je
\emph{konzervatívne rozšírenie} teórie $T$, ak $T'$ je rozšírenie $T$ a pre
každú formulu $A$ na jazyku $L$, pre ktorú platí, že v $T'$ je dokázateľná tá
formula implikuje, že v $T$ je taktiež dokázateľná.

\paragraph{Tvrdenie:} $T$, $T'$ je konzervatívne rozšírenie $T$, potom $T$ je
bezosporná vtedy a len vtedy, keď $T'$ je bezosporná. Vyplýva to z toho, že ak
zoberieme konzervatívne rozšírenie, pre ľubovoľnú formulu pôvodného jazyka
platí, že:
$$ T' \vdash_L A \Rightarrow T \vdash A$$.

\paragraph{Veta (Henkinova):} K ľubovoľnej teórii $T$ môžeme zostrojiť Henkinovu
teóriu $T_H$, ktorá je konzervatívnym rozšírením teórie $T$.

\paragraph{Dôkaz:} Teóriu $T$ rozšírime a priradíme k nej teóriu $T_H$ tak, že
jednak rozšírime jazyk teórie a jednak pridáme axiómy. Nech teória $T$ má jazyk
$L$. Budeme tvoriť jazyk rozšírený jazyk $L_H$ (budeme pridávať konštanty):
...
$$ T, B_1, \ldots B_n \vdash A $$. Môžeme použiť vetu o dedukcii:

$$ T \vdash B_1 \implies B_2 \implies \ldots B_n \implies A $$.
...
v $B_2, \ldots, B_n$. Použijeme vetu o konštantách a dostávame, že $T \vdash
((\exists x) D \implies D_x[c_D] ) \implies (B_2 \implies \ldots \implies B_n
\implies A)$.

\par Ak $T \vdash A \implies B$, potom $T\vdash (\exists x) A \implies B$.
$$ T\vdash (\exists w) ((\exists x)D \implies D_x[w]) \implies (B_2 \implies
...
$$T \vdash ((\exists x) D \implies (\exists w)D_x[w]) \implies B_2 \implies \ldots \implies B_n \implies A$$

Použijeme vetu o variantoch a dostávame:
$$ T \vdash ( \exists x ) D \implies (\exists w) D_x[w] $$
$$ T \vdash B_2 \implies B-3 \implies \ldots \implies B_n \implies A $$
$$ T \vdash A$$
...
\mathcal{B}, S' \mbox{ je konečná}$$


\section{Prednáška 10.3.2009}
...
Tvrdenie dokážeme sporom. Predpokladajme, že $S$ nepatrí do $B$. Potom $T \union S$ je sporná, čiže pre ľubovoľnú dokázateľnú formulu A, $\vdash \neg(A \implies A)$.
...
$T' \vdash A $ alebo $T' \vdash \neg A$.

Nech $T'$ nie je úplná teória. Čiže ani jedno z predošlých nie je dokázateľné. Čiže $\neg A$ sa nenachádza ani v $T$ ani $S_0$. 

Uvažujme, že $T' \union \{\neg A\}$ je bezosporná teória. To je spor s tým, že $S_0$ je maximálny prvok z $\mathcal{B}$ vzhľadom na inklúziu. Pozn.: Odvoláme sa na predtým dokázaný dôsledok: máme $A$, $A'$ je uzáver. Potom $T \vdash A \iff T \union \{\neg A'\}$ je sporná teória.
...
$T'$ je bezosp. rozšírenie $T$, potom $T \vdash A \iff T' \vdash A$ Potom platí, že keď zooberiem ľub. $A \in T$, potom  $m' \vdash A$. Pre ľubov. rozšírenie $T$, $T'$ $A\in T, T' \vdash A \implies m' \models A, m, v T$.
...
Prenexné opperácie:
$$(Qx)A(x) \vee B \equiv (Qx)(A(x) \vee B) \mbox{ - $x$ nie je voľné v $B$} $$
...
Platí toto?:
$$(\forall x) A(x) \vee (\forall X) B \equiv (\forall x) (A(x) \vee B(x))$$
...
$$\cdots$$
$$ (Q_1 x)A(x) \vee (Q_2 x)B(x) \equiv (Q_1 x)(Q_2 z)(A(x) \vee B(z)) $$
...
$I(v)$ obsahuje $A_i$ alebo $\neg A_i$ (ale nie obe naraz). (TODO: check) 
...
\paragraph{Definícia} Vrchol $v$ sémantického (TODO alebo uzavretého???) stromu
...
počet nasledovníkov je nekonečný. (TODO: lepšia formulácia)
...
\vdash A_1 \land A_2 \land \ldots \land A_n \implies B & \\
...
$\neg A \lor V, A \lor C \vdash B \lor C$
$$A \implies B, \neg A \implies C \vdash \neg B \implies C$$
\begin{enumerate}
	\item $$\vdash (A \implies B) \implies (\neg B \implies \neg A)$$
	\item $$ \vdash (\neg B  \implies \neg A) \implies ((\neg A \implies C)
	\implies (\neg B \implies C))$$
\end{enumerate}
$$ A \implies B, \neg C \implies C \vdash \neg V \implies C$$
